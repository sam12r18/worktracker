...
class WorkReportController extends Controller {
 public function index(Request $request,CentralReportingService $reports):View {
  $uid=$request->user()->getKey();$tz=(string)($request->query('timezone')?:config('worktracker.display_timezone','Asia/Tehran'));$preset=(string)($request->query('preset')?:'week');$today=CarbonImmutable::now($tz)->startOfDay();
