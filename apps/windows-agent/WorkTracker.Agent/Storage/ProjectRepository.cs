...
public sealed class ProjectRepository(LocalDatabase database)
{
    ...
    {
        await using var connection = database.OpenConnection(); await using var cmd = connection.CreateCommand(); cmd.CommandText = "SELECT id,project_id,rule_type,pattern,weight,priority,is_enabled FROM project_rules ORDER BY priority DESC, weight DESC";
        var result = new List<ProjectRule>(); await using var reader = await cmd.ExecuteReaderAsync(cancellationToken);
        while (await reader.ReadAsync(cancellationToken)) { if (!Enum.TryParse<ProjectRuleType>(reader.GetString(2), true, out var type)) continue; result.Add(new ProjectRule(reader.GetString(0),reader.GetString(1),type,reader.GetString(3),reader.GetInt32(4),reader.GetInt32(5),reader.GetInt32(6)==1)); }
        return result;
    }
    ...
    public async Task<ProjectRule> AddRuleAsync(string projectId, ProjectRuleType type, string pattern, int weight = 80, int priority = 0, CancellationToken cancellationToken = default)
    {
        ...
        await using (var lc = database.OpenConnection()) await using (var lookup = lc.CreateCommand()) { lookup.CommandText = "SELECT id,project_id,rule_type,pattern,weight,priority,is_enabled FROM project_rules WHERE project_id=$project AND rule_type=$type AND pattern=$pattern LIMIT 1"; lookup.Parameters.AddWithValue("$project", projectId); lookup.Parameters.AddWithValue("$type", type.ToString()); lookup.Parameters.AddWithValue("$pattern", pattern); await using var reader = await lookup.ExecuteReaderAsync(cancellationToken); if (await reader.ReadAsync(cancellationToken)) return new ProjectRule(reader.GetString(0),reader.GetString(1),type,reader.GetString(3),reader.GetInt32(4),reader.GetInt32(5),reader.GetInt32(6)==1); }
        var rule = new ProjectRule(Guid.NewGuid().ToString(), projectId, type, pattern, Math.Clamp(weight,1,200), priority, true);
        ...
        cmd.CommandText = "INSERT INTO project_rules(id,project_id,rule_type,pattern,weight,priority,is_enabled,updated_at) VALUES($id,$project,$type,$pattern,$weight,$priority,1,$updated)";
        cmd.Parameters.AddWithValue("$id",rule.Id); cmd.Parameters.AddWithValue("$project",rule.ProjectId); cmd.Parameters.AddWithValue("$type",rule.Type.ToString()); cmd.Parameters.AddWithValue("$pattern",rule.Pattern); cmd.Parameters.AddWithValue("$weight",rule.Weight); cmd.Parameters.AddWithValue("$priority",rule.Priority); cmd.Parameters.AddWithValue("$updated",DateTimeOffset.UtcNow.ToString("O")); await cmd.ExecuteNonQueryAsync(cancellationToken);
        await InsertOutboxAsync(connection,(SqliteTransaction)tx,"project_rule",rule.Id,new { id=rule.Id, project_id=rule.ProjectId, rule_type=rule.Type.ToString(), pattern=rule.Pattern, weight=rule.Weight, priority=rule.Priority, is_enabled=true, version=1 },cancellationToken); await tx.CommitAsync(cancellationToken); return rule;
    }
