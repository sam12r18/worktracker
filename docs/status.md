...
Current release: **0.1.0-alpha.7.2 — Admin Master Data, Context Help & Agent Reliability**
...
- alpha.7.2: first-class Project/Customer admin UI, Project Rules, project Tasks, pricing override lifecycle/history visibility, Activity Type activation/order, dedicated Access Token UI, route-aware contextual Help, Agent single-instance/build-lock protection and full Project Rule operator support in the Windows classifier.
...
- A real Windows/.NET 10 build is still the authoritative WPF compiler gate. The alpha.7.2 source explicitly aliases WPF MessageBox to avoid the WPF/WinForms ambiguity introduced by the Tray integration.

## Next
- Real Windows build + Agent-to-local-Laravel sync smoke test using a Device Token.
